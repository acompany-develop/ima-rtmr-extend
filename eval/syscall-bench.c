...
static void run_execve(unsigned long n, const char *exe) {
    for (unsigned long i = 0; i < n; i++) {
        ...
        pid_t pid = fork();
        if (pid == 0) { execl(exe, exe, (char *)NULL); _exit(127); }
        waitpid(pid, NULL, 0);
        printf("execve %llu\n", rdtscp() - t0);
    }
}
...
int main(int argc, char **argv) {
    ...
    else if (!strcmp(argv[1], "openat")) { if (argc < 4) return 2; run_openat(n, argv[3]); }
    else if (!strcmp(argv[1], "read"))   { if (argc < 4) return 2; run_read(n, argv[3]); }
    else return 2;
    return 0;
}
